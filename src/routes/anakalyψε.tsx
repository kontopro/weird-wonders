...
export const Route = createFileRoute("/anakalyψε")({ loader: () => articleRepository.listPublished(), validateSearch: z.object({ category: z.string().optional() }), head: () => ({ meta: [{ title: brandedTitle("Ανακάλυψε") }, { name: "description", content: "Αναζήτησε ιστορίες και facts ανά θέμα και χρόνο ανάγνωσης." }, { property: "og:title", content: brandedTitle("Ανακάλυψε") }, { property: "og:description", content: "Βρες την επόμενη ιστορία που θα σε εκπλήξει." }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary_large_image" }] }), component: Discover });
function Discover() {
  const articles = Route.useLoaderData(); const initial = Route.useSearch(); const [query, setQuery] = useState(""); const [category, setCategory] = useState(initial.category ?? "Όλες"); const [duration, setDuration] = useState("Όλοι"); const [sort, setSort] = useState("recent"); const { bookmarks, toggle } = useBookmarks();
